...
def generate_type_mapping(headers_list_filename = './libuing_headers_list.json'):
...
def generate_define_list(headers_list_filename = './libuing_headers_list.json', concatinate = True):
    headers_list_file = Path(headers_list_filename)
...
def _init_define_mapping(mapping_filename = './libuing_define_mapping.json'):
...
def _init_type_mapping(mapping_filename = './libuing_cindex_mapping.json'):
    mapping_file = Path(mapping_filename)
...
def register_libuing_cindex_mapping(strTypeKind, strLibuingTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    if strLibuingTypedef not in cindex_mapping:
        cindex_mapping[strLibuingTypedef] = strTypeKind

def query_libuing_cindex_mapping_entry_exists(strLibuingTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return strLibuingTypedef in cindex_mapping

def get_libuing_cindex_mapping_value(strLibuingTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return cindex_mapping[strLibuingTypedef]


def get_cindex_ctypes_mapping(strTypeKind, strLibuingTypedef):
    # Remove const declaration
    strLibuingTypedef = re.sub(r"^\s*const\s*", '', strLibuingTypedef)

    if strTypeKind == 'TypeKind.RECORD':
        return strLibuingTypedef

    # Handle variable length arguments
    # - Check special marks recoreded in collect_decl_function
    if strTypeKind == 'TypeKind.INVALID' and strLibuingTypedef == 'va_list':
        return ':varargs'

    isPointerToChar = False
    if strTypeKind == 'TypeKind.POINTER':
        pattern = re.compile(r"\s*char\s*\*")
        m = re.search(pattern, strLibuingTypedef)
        if m:
    ...
    if strTypeKind == 'TypeKind.TYPEDEF':
        ...
        if m:
            ...
            if strLibuingTypedef == 'uintptr_t':
                isUintPtrT = True
    ...
    if isPointerToChar:
        ...
        if strTypeKind != 'TypeKind.TYPEDEF':
            ...
            mapping_key = get_libuing_cindex_mapping_value(strLibuingTypedef)

        if mapping_key == "TypeKind.RECORD":
            return str(strLibuingTypedef)
        else:
            return ctypes_mapping[mapping_key]

def is_libuing_callback_type(strTypeKind, strLibuingTypedef):
    return query_libuing_cindex_mapping_entry_exists(strLibuingTypedef) and (get_libuing_cindex_mapping_value(strLibuingTypedef) == "TypeKind.FUNCTIONPROTO")
...
def execute(ctx, arg=[]):
    ...
    try:
        ...
        for typedef_name, typedef_info in ctx.decl_typedefs.items():
            register_libuing_cindex_mapping(str(typedef_info.type_kind), typedef_name)
